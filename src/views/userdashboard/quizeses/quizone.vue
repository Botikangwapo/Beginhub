...
const submitQuiz = async () => {
    ...
    const allAnswered = questions.value.length > 0 &&
        userAnswers.value.length === questions.value.length &&
        !userAnswers.value.includes(undefined);

    if (!allAnswered) {
        alert("Oops! Please answer all questions before submitting.");
        return; // Hihinto ang function at hindi tutuloy sa pag-save
    }
    ...
    console.log("Current User Answers:", userAnswers.value);
        ...
        <div v-else-if="currentQuestion"
            ...
            <div
                ...
                <div class="space-y-3 pt-4 border-t border-blue-50">
                    ...
                    <button @click="submitQuiz" :disabled="isSubmitting" :class="[
                        'w-full font-black py-3 rounded-xl text-xs tracking-widest transition-colors',
                        isSubmitting ? 'bg-blue-200 text-blue-800' : 'bg-blue-600 text-white'
                    ]">
